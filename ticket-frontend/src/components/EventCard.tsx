...
export default function EventCard({ event, index }: EventCardProps) {
    ...
    >
      ...
      <div className="p-5">
        ...
        <div className="flex items-center justify-between">
          ...
          <span className={`text-xs font-medium transition-colors ${isAdmin ? 'text-purple-400' : 'text-purple-400 group-hover:text-cyan-400'}`}>
            {isAdmin ? 'Quản lý →' : 'Đặt vé →'}
          </span>
        </div>
      </div>
  ...
  if (isAdmin) {
    return <Link href="/admin/events">{cardContent}</Link>;
  }
